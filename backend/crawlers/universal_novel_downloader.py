...
================================================================================
用途:
    与 legacy_pachong 中的下载器逻辑保持一致/可复用，用于后端服务场景复用，
    支持从章节JSON批量抓取并生成报告，便于与Django流程对接。
说明:
    若存在功能差异，以同名模块的最新实现为准；本文件用于后端场景快速接入。
...
class UniversalNovelDownloader:
    ...
    async def download_chapters(self, chapters, site_key=None, output_dir=None):
        ...
        async with async_playwright() as p:
            browser = await p.chromium.launch(headless=browser_config.get('headless', True))
